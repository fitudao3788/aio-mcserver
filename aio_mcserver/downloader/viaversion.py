...
class ViaVersion(BaseDownloader):
    filename_regex = r"^ViaVersion-(\d+(?:\.\d+)*)\.jar$"

    async def get_download_info(self) -> dict:
        info_res = await self.client.get("https://hangar.papermc.io/api/v1/projects/ViaVersion/versions?limit=1&offset=0&channel=Release&platform=PAPER&includeHiddenChannels=true")
